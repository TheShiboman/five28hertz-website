...
import { Star, Plus, Trash2, ThumbsUp, Calendar, Pencil, X } from "lucide-react";
import { formatDistanceToNow } from 'date-fns';

interface PropertyReviewsProps {
  propertyId: number;
  bookingId?: number; // Optional booking ID if reviewing after a stay
...
export function PropertyReviews({ propertyId, bookingId, showAddReview = true, className = '' }: PropertyReviewsProps) {
  const { user } = useAuth();
  ...
  const { data: reviews, isLoading } = useQuery<ReviewWithAuthor[]>({
    ...
    queryFn: async () => {
      const res = await apiRequest('GET', `/api/reviews/property/${propertyId}`);
      return res.json();
    },
    enabled: !!propertyId,
  });
      ...
      {isFormOpen && (
        <Card className="bg-muted/50">
          <CardHeader className="pb-2">
            ...
            <CardDescription>Share your experience staying at this property</CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <CardHeader className="pb-2">
                <div className="flex justify-between items-start">
                  <div className="flex items-center gap-2">
                    <Avatar>
                      <AvatarImage src={review.authorImage || ""} alt={review.authorName || "Guest"} />
                      <AvatarFallback>{(review.authorName || "G").charAt(0)}</AvatarFallback>
                    </Avatar>
                    <div>
                      <CardTitle className="text-base">{review.authorName || "Anonymous Guest"}</CardTitle>
